...
- packages: collection of function and data
- data frame is an object that stores data and it has variables and observations. 
- variables are columns of the data frame
- observations are rows

2.  What's a function?
    What's an argument?

-   `glimpse(data)`: glimpse is the function, data is the argument
-   `mean(x)`, mean is the function, x is the argument
-   `library(tidyverse)`, library is the function, tidyverse is the argument  
...
813 observations (rows) and 5 variables (columns)
...
Let's use `distinct` to find the number of different time points water was sampled. 3 times the water is sampled.

```{r count-draws}
distinct(flint, draw)
```

How many unique homes are in the data set? 269 homes in the data set.
...
-   A note on pipes `|>` and `%>%`. A pipe takes what comes before it and makes it the first argument of the following function. Read a pipe as "and then"
...
Which ZIP code had the most samples drawn? 48503

"Take the flint data set and then count the number of each zip code"
